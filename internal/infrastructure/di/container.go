...
func NewContainer(ctx context.Context, cfg *config.Config, log *slog.Logger) (*Container, error) {
	...
	alertsCfg, err := config.BuildAlertsEngineConfig(cfg.AlertsEngineSettings())
	if err != nil {
		return nil, fmt.Errorf("alerts config: %w", err)
	}
	alertEngine, err := alerts.New(alertsCfg, bus, clk)
	if err != nil {
	...
	feedbackCfg, err := config.BuildFeedbackEngineConfig(cfg.FeedbackEngineSettings())
	if err != nil {
		return nil, fmt.Errorf("feedback config: %w", err)
	}
	feedbackEngine, err := feedback.New(feedbackCfg, bus, clk)
	if err != nil {
